...
#[ repr( C ) ]
#[ derive( Default, NoUninit, Clone, Copy ) ]
struct UniformRaw
{
  view_matrix : [ f32; 16 ],
...
pub struct IBLRendererCube
{
  env_map : Rc< CubeTexture >,
  diffuse_texture : CubeTexture,
  specular_1_texture : CubeTexture,
...
impl IBLRendererCube 
{
  ...
  { 
    ...
    let diffuse_texture = CubeTexture::new( device, format, diffuse_width, diffuse_height, false );
    let specular_1_texture = CubeTexture::new( device, specular_1_format, specular_1_width, specular_1_height, true );
    ...
    ( 
      ...
      { 
        label: None, 
        source: wgpu::ShaderSource::Wgsl( include_str!( "shaders/ibl2.wgsl" ).into() )
      }
    );
  ...
  pub fn render_diffuse( &self, encoder : &mut wgpu::CommandEncoder, device : &wgpu::Device, queue : &wgpu::Queue, )
  {
    let mut uniforms = UniformRaw::default();
    let projection = glam::Mat4::perspective_rh_gl( 90.0f32.to_radians(), 1.0, 0.1, 10.0 );
    let view_transforms = 
    [
      glam::Mat4::look_to_rh( glam::Vec3::ZERO, glam::Vec3::X, glam::Vec3::NEG_Y ),
      glam::Mat4::look_to_rh( glam::Vec3::ZERO, glam::Vec3::NEG_X, glam::Vec3::NEG_Y ),
      glam::Mat4::look_to_rh( glam::Vec3::ZERO, glam::Vec3::NEG_Y, glam::Vec3::NEG_Z ),
      glam::Mat4::look_to_rh( glam::Vec3::ZERO, glam::Vec3::Y, glam::Vec3::Z ),
      glam::Mat4::look_to_rh( glam::Vec3::ZERO, glam::Vec3::Z, glam::Vec3::NEG_Y ),
      glam::Mat4::look_to_rh( glam::Vec3::ZERO, glam::Vec3::NEG_Z, glam::Vec3::NEG_Y ),
    ];

    uniforms.inv_projection_matrix = projection.inverse().to_cols_array();
    for i in 0..6
    {
      let diffuse_view = &self.diffuse_texture.create_mip_view( i, 0 );
      uniforms.view_matrix = view_transforms[ i as usize ].to_cols_array();
      uniforms.mip_level = i;
    ...
    (
      ...
      wgpu::TexelCopyBufferInfo
      {
        ...
        {
          offset : 0,
          bytes_per_row : Some( self.diffuse_texture.mip_memory_size_row( 0 ) ),
          rows_per_image : Some( self.diffuse_texture.size().height )
        }
      },
  ...
  pub fn render_specular_1( &self, encoder : &mut wgpu::CommandEncoder, device : &wgpu::Device, queue : &wgpu::Queue )
  {
    let mut uniforms = UniformRaw::default();
    let projection = glam::Mat4::perspective_rh_gl( 90.0f32.to_radians(), 1.0, 0.1, 10.0 );
    let view_transforms = 
    ...
    uniforms.inv_projection_matrix = projection.inverse().to_cols_array();
    uniforms.total_mips = self.total_mips;
    ...
    for mip_level in 0..self.total_mips
    {
      ...
      for i in 0..6
      {
        ...
        let mut encoder = device.create_command_encoder( &wgpu::CommandEncoderDescriptor::default() );
        let view = self.specular_1_texture.create_mip_view( i, mip_level );
  ...
  pub async fn save_diffuse( &self, device : &wgpu::Device )
  {
...

    {
      ...
      encoder.write_image( &data, size.width, size.height * size.depth_or_array_layers, image::ExtendedColorType::Rgb32F ).unwrap();
    }
  ...
  pub async fn save_specular_1( &self, device : &wgpu::Device )
  {
    ...
    for mip_level in 0..self.total_mips
    {
...

      {
        ...
        let mut data = Vec::with_capacity( ( wrapper.unpadded_bytes_per_row * rows ) as usize );
        for row in 0..rows
        {
